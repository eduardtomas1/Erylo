...
The dependency direction is app -> windowing -> surface/integrations -> core. Platform types do not leak into the domain model.
...
The foundation test harness intentionally uses only the standard Swift toolchain. The current standalone Command Line Tools distribution does not expose `XCTest` or Swift Testing through SwiftPM, so `Scripts/ci.sh` builds every product and runs the dependency-free unit harness. A later Xcode project can add XCTest/XCUITest without changing the pure test seams.
...
- No activity providers are enabled, and no broker, permission flow, persistence, updater, signing, or diagnostics pipeline is claimed here.
