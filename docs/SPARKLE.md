...
The current app starts the configured updater but never initiates a check. An injected `UpdateDriving` seam preserves a future user-initiated check without coupling release infrastructure to settings or panel UI. The update harness covers Boolean, string, and numeric plist/default variants, preexisting preferences, simulated managed values that resist writes, factory/start failure, and the manual-check seam. Automatic network work remains mechanically disabled; manual checks remain user-initiated only.
