...
- Glance providers are not connected to the app-owned broker or settings yet, so the app enables none of them. Calendar restore/start never requests permission; contextual acquisition is a separate explicit seam followed by a prompt-free enable/resume. Battery and distant meetings create no resting broker activity, while visible timer progress uses bounded second-level successor one-shots. See [GLANCE_LIFECYCLE.md](GLANCE_LIFECYCLE.md). The app creates one shared activity broker and injects one observable subscriber model across display panels; the model starts only while the coordinator is enabled and running. Settings mounting, persistence, updater, signing, and application diagnostics wiring are not claimed here.
