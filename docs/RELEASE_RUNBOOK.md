...
- `Config/ProductionCapabilities.json`: the closed, reviewed mapping from the mounted Focus Timer, Battery, and Volume composition to privacy usage descriptions and capability entitlements. Both declaration allowlists are empty today. Canonical tree hashes bind every Swift source path and byte in the capability-bearing Glance and media modules, so indirection changes require an explicit policy review.
- `Resources/App/Info.plist.in`: reviewed bundle, category, agent-app, and default-off updater metadata; it contains no privacy usage description for unmounted utilities.
- `Resources/App/Erylo.entitlements`: the intentionally empty capability-entitlement input. Developer ID signing must leave it empty; the team derived from the explicitly selected Developer ID identity is checked separately against signature metadata.
...
- `Config/ReleaseCompilerInputs.txt`: the sorted, deny-by-default set of Git blobs that Swift may compile into the production executable. Repository and release gates derive the exact recursive local target closure for the `Erylo` product from SwiftPM and require equality with this policy. Any transient, omitted, noncanonical, or otherwise unreviewed in-image Swift input aborts the build.
...
- Available exact `Developer ID Application: ORGANIZATION (TEAMID)` identity in the current Keychain, where `TEAMID` is ten uppercase alphanumeric characters.
...
The identity and profile names are selectors, not credentials. The worker derives the expected team only from the exact explicit Developer ID identity, exports that value through the signing/notarization stages, and rejects any selected identity or signed app whose `TeamIdentifier` differs. Direct signing may supply the same value through `ERYLO_RELEASE_EXPECTED_TEAM_ID`; if present, it must have the exact ten-character uppercase alphanumeric shape and match the identity. The wrapper rejects a dirty or unpushed/untracked source state, any mid-run live-worktree mutation, missing inputs, Command Line Tools-only environments, missing identities/profiles/tools, invalid public feed/key metadata, signing failures, non-accepted notarization, stapling failures, Gatekeeper failures, and missing Sparkle Keychain keys. Production scripts use `/bin/bash`, reset to a closed system PATH, and resolve every Apple developer tool through `/usr/bin/xcrun` under one exported, revalidated `DEVELOPER_DIR`. Bundle and public/private manifests bind the source commit, source tree, appcast-config SHA-256, and canonical toolchain-manifest SHA-256.
...
5. `assemble-app.sh` first binds the production composition to the reviewed capability policy, then creates the fixed bundle layout, embeds Sparkle and the canonical toolchain provenance, copies the exact reviewed licenses/notices, removes unused XPC services, routes a real icon only when supplied, and injects public signed-feed metadata and its pinned config hash only when complete.
6. `validate-app.sh --require-updater` checks plist values, the exact privacy/entitlement allowlists, source/config/version/architecture consistency, exact licenses/notices, rpaths, Sparkle version/layout, symlink containment, and secret/placeholders.
7. `sign-app.sh` derives the team from the exact explicit Developer ID identity, requires any inherited expected-team value to match, uses Sparkle's documented inner-to-outer signing order with secure timestamps and Hardened Runtime, and then proves the signed app's `TeamIdentifier` equals that derived team. It never signs with `--deep` or `-`.
8. `verify-signature.sh --pre-notarization` verifies every nested component, the app, Developer ID authority, runtime flag, signed entitlements, and the exact expected `TeamIdentifier`; the same inherited binding is rechecked after notarization and stapling.
