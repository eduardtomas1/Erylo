...
[[ -d "$app" && "$(basename "$app")" == "Erylo.app" ]] || release_die "input is not a staged Erylo.app bundle"
...
assert_plist_equals EryloSourceCommit "$(git rev-parse --verify HEAD)"
...
assert_plist_equals NSAppleEventsUsageDescription "Erylo sends playback commands to supported media apps only when you use media controls."
...
[[ "$(lipo -archs "$binary")" == "arm64" ]] || release_die "main executable is not arm64-only"
minos="$(otool -l "$binary" | /usr/bin/awk '$1 == "minos" { print $2; exit }')"
...
otool -l "$binary" | /usr/bin/grep -Fq '@executable_path/../Frameworks' \
...
lipo -archs "$framework/Versions/B/Sparkle" | /usr/bin/tr ' ' '\n' | /usr/bin/grep -Fxq arm64 \
...
if [[ -n "$feed_url" || -n "$public_key" ]]; then
    ...
    /usr/bin/ruby -rbase64 -e '
        key = ARGV.fetch(0)
        abort if key.match?(/replace|placeholder|todo/i)
        abort unless Base64.strict_decode64(key).bytesize == 32
      ' "$public_key" || release_die "Sparkle public key metadata is invalid"
