...
This repository contains a Swift 6, macOS 14+ SwiftPM foundation. `Scripts/ci.sh` is the canonical entry point: it builds with warnings as errors, runs the activity, foundation, glance, trust, updater, and release harnesses, and uses `shellcheck` when that optional local tool is installed.
