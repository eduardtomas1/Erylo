...
The foundation, activity, File Hold, glance, media, trust, integration, surface, and updater harnesses intentionally use executable test targets rather than `XCTest` or Swift Testing, which the current standalone Command Line Tools distribution does not expose through SwiftPM. `Scripts/ci.sh` builds every product with warnings as errors, checks the activity and media API surfaces, runs every Swift harness, and then runs the release shell harness. A later Xcode project can add XCTest/XCUITest without changing the pure test seams.
...
- Glance providers are not connected to the app-owned broker or settings yet, so the app enables none of them. Calendar restore/start never requests permission; contextual acquisition is a separate explicit seam followed by a prompt-free enable/resume. Battery and distant meetings create no resting broker activity, while visible timer progress uses bounded second-level successor one-shots. See [GLANCE_LIFECYCLE.md](GLANCE_LIFECYCLE.md). The app creates one shared activity broker and injects one observable subscriber model across display panels; the model starts only while the coordinator is enabled and running. Settings mounting, persistence, signing, and application diagnostics wiring are not claimed here; the updater wiring remains default-off without signed feed metadata.
...
- Trust persistence, contextual permission/lifecycle seams, and local diagnostics export now exist as unmounted library foundations. A settings-window host and panel preference integration remain later work.
